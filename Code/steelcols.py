...
# Find start and end points for columns in design section list
...
# Find omega 2 for bending calculations
def omega2 (desInfo):
    # Moments on column assumed to be applied only at top, none at intermediate points
    kappa = min([desInfo[2], desInfo[3]])/max([desInfo[2], desInfo[3]])
    w2 = 1.75 + 1.05*kappa + 0.3*kappa**2
    if w2 > 2.5:
        w2 = 2.5
    return w2

# Calculates U1 for beam-column checks
...
def ULS ( desInfo, shapes, column ):
    ...
    # Capacity check a
        ...
        efficiency.append(desInfo[1]/column.Cr + (0.85*U1x*desInfo[2])/(0.9*column.Mpx)+(beta*U1y*desInfo[3])/(0.9*column.Mpy))
        ...
        efficiency.append(desInfo[1]/column.Cr + (0.85*U1x*desInfo[2])/(0.9*column.Mpx)+(beta*U1y*desInfo[3])/(column.Mry))
        
        # Capacity check c
        column.k = tempk
        U1x, U1y = UCalc(3, desInfo[1], column, kappa)
        efficiency.append(desInfo[1]/column.Cr + (0.85*U1x*desInfo[2])/(column.Mrx)+(beta*U1y*desInfo[3])/(column.Mry))

        # Capacity check d
        efficiency.append(desInfo[2]/column.Mrx + desInfo[3]/column.Mry) 

    # Check if no moment is applied (speeds up calculation time)
    else:
        column.CrCalc(desInfo, 1)
        # Final check for if column passes ULS
        efficiency.append(desInfo[1]/column.Cr)
    eMax = max(efficiency)
    return column, eMax

# SLS check for slenderness
...
# Checks all columns of selected type for ULS and SLS resistance
def mainloop(desInfo, shapes):
    ...
    for i in range (st, en):
        ...
        if efficiency < 0.5 or efficiency >= 0.9:
            continue
        elif column.secClass == 4: # Decided we just don't want class 4 columns, so all class 4 sections ignored
            continue
        elif klr > 200: # Slenderness check 
            continue
        else:
    ...
    try:
        fileName = desInfo[6] # desInfo[6] only exists in auto design case, so an error occurs in single design case - going to the single design output
        return column    
    except:
        # Outputs designed column for single column design
        print("Designed ", column.name, ", Cr = ", column.Cr, ", Mrx = ", column.Mrx, ", Mry = ", column.Mry)
...
# Writes design calculations to file (Only for non-moment case)
...
# Auto design takes column loads from a csv file instead of user inputs. Otherwise works exactly the same
if auto.upper() == "Y" or auto.upper() == "":
    fInput = []
    desInfo = []
    autoName = input("Name of file to import from: ")
    in_file = "../Assets/{}.csv".format(autoName)
    out_file = open("../Output/output.csv", "w")
    writer = csv.writer(out_file)
    with open(in_file, 'r') as autoSheet:
        ...
        for z in range(1, len(fInput)):
            ...
            column = mainloop(desInfo, shapes)
            try:
                row = [fInput[z][0],column.name, column.Cr, column,Mrx, column.Mry]
            except:
                row = [fInput[z][0], column.name, column.Cr]
            writer.writerow(row)
    autoSheet.close()
    out_file.close()
else:
    # colProperties function runs user through load input
